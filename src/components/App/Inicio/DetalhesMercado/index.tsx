...
export default function DetalhesMercado({ navigation, route }: DetalhesMercadoProps) {
    ...
    const [enderecoGeocode, setEnderecoGeocode] = useState<ResultadoGeoCode>();
    const [mapaCarregando, setMapaCarregando] = useState<boolean>(false);
    ...
    const obterEnderecoGeocode = async () => {
        setMapaCarregando(true);

        try {
            const { data } = await mapaServices.search({ consulta: Formatador.formatarEnderecoMercado(item) });
            setEnderecoGeocode(data);
        }
        catch (erro) {
            console.log(erro);
        }
            ...
            <View style={estilos.mapaContainer}>
                {mapaCarregando ?
                    ...
                    <MapView
                        ...
                        customMapStyle={temaMapa}
                        initialRegion={{
                            latitude: enderecoGeocode?.results[0].position.lat || -23.550520,
                            longitude: enderecoGeocode?.results[0].position.lon || -46.633308,
                    ...
                    >
                        <Marker
                            ...
                            title={item.nome}
                            description={Formatador.formatarEnderecoMercado(item)}
                        />
                    </MapView>
                }
            </View>
